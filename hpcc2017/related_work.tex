...
The study of fault tolerance in HPC has been the focus of research, with significant progress on how we mitigate the impact of failures~\cite{herault2015fault}. %The main focus has been on incorporating fault tolerance into MPI, the de facto programming paradigm for HPC. 
...
each process maintain multiple checkpoints, necessary to construct a consistent state during recovery, and complicates the garbage collection scheme~\cite{Elnozahy:02:Survey}. %It can also suffer the well-known domino effect~\cite{randell_domino_effect}. %One hybrid approach, known as communication induced 
...
One of the largest overheads in disk-based checkpointing techniques is the time to save a checkpoint
to a stable storage. %, such as a centralized file system. 
