...
Fortunately, slowing down the shadows provides an opportunity for the shadows to benefit from the faster execution of their mains. By copying the state of each main to its shadow, which is similar to the process of storing a checkpoint in a buddy in \cite{6264677}, forward progress is achieve for the shadows with minimized time and energy. The so called \textit{shadow leaping} effectively limits the distance in progress between main and shadow, which simultaneously reduces the recovery time for failures.
