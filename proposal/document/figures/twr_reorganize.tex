...
To obtain the chip maps, we use the VARIUS tool \cite{SM08:varius}, which is a novel microarchitecture-aware model for both within-die (WID) and die-to-die (D2D) process variations. 
%We focus on WID in this paper. 
...
For D2D variations
%\footnote{Different from \cite{DATE15:twr}, we take D2D variation into consideration in this article.}
...
And, random variation is also modeled with normal distribution. %The two distributions are independent and each has its own sigma, denoted as $\sigma_{sys}$ and $\sigma_{rand}$ respectively; hence, the overall normal distribution can be achieved by superposing both components, i.e. the overall standard variation is $\sigma_{total}=\sqrt{\sigma_{sys}^2+\sigma_{rand}^2}$.
...
\subsection{Chip-specific tWR control}
...
%{\footnote{A chip-row refers to the portion of cells of a row that reside on one chip. A DIMM-row refers to all the cells of a row.}}
...
\subsection{Constructing Fast Chunks through Chunk Remapping}
...
%To enable variation aware memory scheduling, we added two tables: one is for extracting the timing constraints of each chunk while the other is for chunk address remapping. 
...
\subsubsection{20nm Technology Node}
%We simulated benchmarks under both random and hot page allocation policies, which are separately reported in Figure \ref{fig:tech_time}.
...
\centering
\includegraphics[width=\linewidth]{figures/TODAES_data/20nm/{stat_RAND_main-cycles.perc.dat}.pdf}\\
\subcaption{20nm normalized average execution time}\label{fig:20nm_time}
\end{minipage}%

\begin{minipage}[b]{0.7\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/TODAES_data/14nm/{stat_RAND_main-cycles.perc.dat}.pdf}\\
\subcaption{14nm normalized average execution time}\label{fig:14nm_time}
\end{minipage}%
\vspace{-0.4in}
\caption{The execution time comparison of different schemes for 20nm and 14nm technology nodes under random page allocation policy. Representative applications and the geometric means for highly memory-intensive (Spec-High) and all applications (Spec-All) are presented here.}
\label{fig:tech_time}
\end{figure}

From Figure \ref{fig:20nm_time}, we observed that 
...
With more slow cells at 14nm, both ECC and Sparing become less effective and their impacts on performance become smaller. {\tt ChunkSort-4k} shows a tendency of better improvement over traditional solutions and {\tt Chunk-4k}. For Spec-High, {\tt ChunkSort-4k} executes 7\% faster comparing to {\tt Spare-8} and {\tt ECC}. {\tt ChunkSortBin-4k} achieves over 20\% performance improvements over {\tt Spare-8} and {\tt ECC}, and 27.5\% over {\tt Relax-100}.

\subsection{Page Allocation Effects}

Figure \ref{fig:tech_time_20nm} and \ref{fig:tech_time_14nm} compare the results using random and restore-time-aware page allocation schemes at 20nm and 14nm technology nodes, respectively.
From the figure, by making better use of fast chunks, restore time aware page allocation speeds up the execution of all chunk based schemes, e.g., for {\tt ChunkSortBin-4k}, restore time aware allocation achieves 10\% and 15\% improvement over random allocation for 20nm and 14nm nodes respectively.
...
\centering
\includegraphics[width=\linewidth]{figures/TODAES_data/20nm/{RAND_small.dat}.pdf}\\
\subcaption{With random page allocation}\label{fig:20nm_rand}
\end{minipage}%
\begin{minipage}[b]{0.48\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/TODAES_data/20nm/{PROF_small.dat}.pdf}\\
\subcaption{With restore time aware page allocation}\label{fig:14nm_prof}
\end{minipage}%
\vspace{-0.4in}
\caption{The execution time comparison of different schemes at 20nm technology node.}
\label{fig:tech_time_20nm}
\end{figure}
...
Also in the figure, we observed that the effectiveness of restore time aware rank construction is diminishing after adopting restore time aware allocation. For example, on average, {\tt ChunkSort-4k} and {\tt ChunkSortBin-4k} have less than 1\% difference when using restore time aware allocation at 20nm node. 
...
\section{Conclusion}
In this work, we studied DRAM scaling effects on restore time, and showed that future DRAM chips need relaxed timing constraints to maintain high yield rates and to keep the manufacturing cost low. Existing approaches are ineffective to address the performance losses. 
