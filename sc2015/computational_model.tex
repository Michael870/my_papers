...
The main process executes at rate $\sigma_m$, while the shadow initially executes at rate $\sigma_s^b \le \sigma_m$. %To avoid simultaneous failure, they are deployed on different cores.
...
\begin{figure}[!t]
	\begin{center}
		...
		{
			\label{fig:succ}
			\includegraphics[width=0.8\columnwidth]{Figures/succ_new.pdf}
		}
		\subfigure[Main process failure.]
		{
			\label{fig:fail}
			\includegraphics[width=0.8\columnwidth]{Figures/fail_new.pdf}
		}
	\end{center}
	%\vskip -0.25in 
